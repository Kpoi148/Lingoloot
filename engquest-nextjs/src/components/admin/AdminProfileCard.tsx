...
export default function AdminProfileCard({
    ...
    return (
        <div className="relative overflow-hidden rounded-3xl border border-slate-200 bg-white p-6 shadow-lg shadow-slate-200/60">
            <div className="absolute right-0 top-0 h-32 w-32 -translate-y-1/2 translate-x-1/3 rounded-full bg-slate-100 opacity-50 blur-2xl" />
                    ...
                    <div className="group relative h-28 w-28">
                        ...
                        {!isEditing && (
                            <InventoryModal
                                ...
                                trigger={
                                    <button className="absolute bottom-0 right-0 z-20 flex h-8 w-8 items-center justify-center rounded-full border border-slate-200 bg-white text-slate-600 shadow-md transition hover:bg-slate-50 hover:text-blue-600">
                ...
                <div className="flex-1 space-y-3">
                            ...
                            {isEditing ? (
                                <div className="flex gap-2">
                                    <input
                                        ...
                                        className="rounded-xl border border-slate-300 px-3 py-1 text-xl font-bold text-slate-900 focus:border-slate-900 focus:outline-none"
                                    />
                                </div>
                            ) : (
                                <div className="flex items-center gap-2">
                                    <h2 className="text-2xl font-bold text-slate-900">{profile.displayName}</h2>
                                    <span className="rounded-full bg-slate-100 px-2 py-0.5 text-xs font-bold text-slate-500">ADMIN</span>
                                </div>
                            )}
                            ...
                            {isEditing ? (
                                <>
                                    ...
                                    <button
                                        onClick={handleSave}
                                        disabled={isSaving}
                                        className="flex h-9 items-center gap-2 rounded-full bg-slate-900 px-4 text-xs font-bold text-white shadow-md hover:bg-slate-800 disabled:opacity-70"
                                    >
                                ...
                                <button
                                    ...
                                    className="flex h-9 items-center gap-2 rounded-full border border-slate-200 bg-white px-3 text-xs font-bold text-slate-600 transition hover:-translate-y-0.5 hover:shadow-sm"
                                >
                    ...
                    {isEditing && (
                        <div className="mt-4 animate-in fade-in slide-in-from-top-2">
                            <p className="mb-2 text-xs font-bold uppercase tracking-wider text-slate-400">Đổi Avatar</p>
                            <MediaUploader
                                mediaType="image"
